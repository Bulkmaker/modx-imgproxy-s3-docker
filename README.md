...
**ресайз, WebP и кеширование** делает [imgproxy](https://imgproxy.net) на
...
конвертит в WebP. Итог: **−80…95 % веса** для крупных JPEG/PNG, кеш на 30 дней,
диск VPS не пухнет (оригиналы на дешёвом S3, локально — только небольшой кеш с
потолком).
...
| `imgproxy` | `darthsim/imgproxy` | ресайз/WebP из S3 |
...
- MODX 3 (скачиваете на шаге 2). Для вызова сниппета через `| snippet` нужен
  **pdoTools**; без него используйте обычный тег `[[imgproxy? …]]` (см. шаг 6)
...
`S3_BUCKET` — это **только имя бакета**, без `s3://` и без слэшей (например
`my-media-bucket`). Оно автоматически подставится в nginx-конфиг при старте
контейнера (через `envsubst`), руками править конфиг не нужно.
...
Версию возьмите актуальную со страницы загрузки (на момент написания — 3.2.1):

```bash
curl -L -o modx.zip "https://modx.com/download/direct?id=modx-3.2.1-pl.zip"
...
> Синтаксис `{'imgproxy' | snippet : […]}` — это **pdoTools**. На чистом MODX 3
> без pdoTools вызывайте сниппет обычным тегом:
> ```
> <img src="[[imgproxy? &src=`[[+file.url]]` &w=`640`]]">
> ```
...
- **Формат закреплён на WebP.** В URL добавляется суффикс `@webp`, поэтому imgproxy
  всегда отдаёт WebP — детерминированно и безопасно для кеша. imgproxy умеет и
  AVIF: чтобы отдавать его по `Accept`, уберите `@webp` из rewrite в `nginx.conf`
  **и** добавьте формат в `proxy_cache_key`, иначе в кеше перемешаются форматы.
- **Base-compose — для локалки, не для голого VPS.** Базовый `docker-compose.yml`
  публикует порты MySQL и phpMyAdmin (root с паролем из `.env`). На сервере
  поднимайте через прод-оверлей (`-f docker-compose.prod.yml`) — он их закрывает.
- **Топология в проде.** Здесь `imgproxy` собран в один compose для простоты.
  Если сайтов несколько — вынесите `imgproxy` в общий стек на внешней сети `web`
  (как у нас в проде) и уберите из compose каждого сайта; все ходят к одному
  `http://imgproxy:8080`. nginx резолвит upstream в рантайме (`resolver 127.0.0.11`),
  поэтому переживёт его перезапуск. Для TLS — оверлей с Traefik (раздел выше).
