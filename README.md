...
│       ├── nginx.conf        # MODX friendly URLs + маршрут /img/ + кеш
...
- **Прод за обратным прокси.** На проде перед `app` обычно стоит Traefik/Cloudflare
  и терминирует TLS; в этом стартере для простоты порт `app` публикуется напрямую.
